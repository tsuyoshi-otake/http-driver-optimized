...
export type OnRequestHook = (info: {
    url: string;
    method: string;
    serviceId: string;
    timestamp: number;
}) => void;
export type OnResponseHook = (info: {
    url: string;
    method: string;
    serviceId: string;
    status: number;
...
export interface DriverConfig {
    ...
    handleInterceptorErrorAxios?: (axiosInstance: unknown, processQueue: (error: unknown, token: string | null) => void, isRefreshing: {
        value: boolean;
    }, addToQueue: (resolve: (value: unknown) => void, reject: (reason: unknown) => void) => void) => (error: unknown) => Promise<unknown>;
    addTransformResponseFetch?: (response: ResponseFormat) => ResponseFormat;
    addRequestTransformFetch?: (url: string, requestOptions: Record<string, unknown>) => {
        url: string;
